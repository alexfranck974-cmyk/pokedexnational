...
interface DexGroup { dexNum: number; cards: TcgCardRow[]; }
...
interface Props {
  ...
  cardsCountLabel: (n: number) => string;
  searchPlaceholder: string;
  notFoundLabel: string;
}

// Generic "curated slice of the TCG index, browsed Pokédex-order" panel —
// shared by CharacterRarePanel ("Duos") and TagTeamPanel ("Tag"), which just
// supply their own `cards` query and copy. Same spirit as TrainersPanel but
// grouped by dex_num instead of by character name, with a heart to wishlist a
// card straight from here instead of an owned/unowned toggle.
export function DexCardPanel({ cards, cardsLoading, userId, refreshControl, countLabel, cardsCountLabel, searchPlaceholder, notFoundLabel }: Props) {
  ...
  const groups = useMemo(() => {
    ...
    return Array.from(byDex.entries())
      .map(([dexNum, groupCards]) => ({ dexNum, cards: groupCards }) as DexGroup)
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>{countLabel(groups.length)}</Text>
      ...
      {cardsLoading ? (
        ...
        <Text style={styles.empty}>{notFoundLabel}</Text>
      ) : (
        <FlashList
          data={visibleGroups}
            ...
            return (
              <View style={styles.row}>
                <View style={styles.info}>
                  ...
                  <Text style={styles.sub} numberOfLines={1}>{cardsCountLabel(item.cards.length)}</Text>
                </View>
